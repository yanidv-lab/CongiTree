...
// prompt/quality logic so a standalone app and a server-backed deployment behave the same way.
...
// Fallback learning tree builder, used only if Gemini is unreachable or returns unparsable JSON.
...
// standalone Android build produces trees matching the scope the user picked, same as the
// server-backed deployment.
function depthGuidanceFor(depthLevel: 'basic' | 'comprehensive' | 'mastery') {
  if (depthLevel === "basic") {
...
export async function generateLearningTreeClient(
...
): Promise<{ success: boolean; tree: LearningTree; isFallback?: boolean }> {
  ...
  const depthGuidance = depthGuidanceFor(depthLevel as any);
  const isHe = language === 'he';
...
- All titles, descriptions, and checklist items must be in ${isHe ? 'Hebrew' : 'English'}.`;
    ...
    return { success: true, isEndOfTopic: subNodes.length === 0, subNodes };
  } catch (err: any) {
    if (err?.message === "GEMINI_API_KEY_MISSING") throw err;
