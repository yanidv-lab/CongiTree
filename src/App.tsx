  ...
  const handleGenerateTree = async (
    ...
    try {
      try {
        ...
        // Direct server API unavailable, fallback to client-side standalone execution
      }

      if (!data || !data.success || !data.tree) {
        data = await generateLearningTreeClient({
          topic,
          language,
          depthLevel,
          customInstructions,
      ...
      if (!data || !data.success || !data.tree) {
        throw new Error('נכשלה יצירת עץ הלמידה');
      }
      ...
      if (err?.message === 'GEMINI_API_KEY_MISSING') {
        ...
        setErrorMessage('אנא הזן מפתח Gemini API בהגדרות כדי ליצור עצי למידה באפליקציה.');
      } else {
        setErrorMessage(err.message || 'שגיאה ביצירת עץ הלמידה');
      }
    } finally {
  ...
  const handleExpandNode = async (node: TreeNode) => {
    ...
    try {
      ...
      if (!data || !data.success) {
        data = await expandTreeNodeClient({
          treeTopic: currentTree.topic,
          nodeId: node.id,
          nodeTitle: node.title,
          nodeDescription: node.description,
      ...
      if (!data || !data.success) {
        throw new Error('נכשלה הרחבת הענף');
      }
